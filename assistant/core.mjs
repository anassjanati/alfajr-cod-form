...
function handoff(reply) { return /whats\s*app|واتس/i.test(reply) && !reply.includes('https://wa.me/212650512222') ? reply + '\nhttps://wa.me/212650512222' : reply; }
...
export function createAssistant({ catalog, budget, apiKey, model, fetcher = fetch }) {
  ...
  return async rawInput => {
    ...
    const wantsLocation = /localisation|adresse|address|location|فين|العنوان|الموقع|fin\s+(?:kayn|jat|kayna)|win\s+|(?:passer|venir|visiter).{0,35}(?:biblioth|magasin|boutique)|n9der\s+nji|نجي/i.test(question);
    ...
    const bookRequest = /\b(?:ktab|ketab|livres?|books?|manuel|mo9arar|tajwid|robert|etincelle)\b|كتاب|مقرر/i.test(input.message);
    if (bookRequest) {
      const vague = /^(?:bghit\s+(?:wah[ae]d\s+)?(?:ktab|ketab)|i want to ask about some books.*)$/i.test(input.message.trim());
      return {mode:'info',topic:'reference',products:[],reply:vague
        ? (isDarija(input.message) ? 'شنو سميت الكتاب اللي كتقلب عليه؟ إلا عندك العنوان والطبعة عطيني إياهم.' : 'Which book are you looking for? Please share its title and edition if you have them.')
        : (isDarija(input.message) ? 'فهمت المرجع اللي كتقلب عليه. التوفر والثمن ديال هاد الكتاب خاص الفريق يأكدهم؛ تقدر تصيفط ليهم نفس المرجع هنا: https://wa.me/212650512222' : 'Pour ce livre, notre équipe doit confirmer la disponibilité, l’édition et le prix. Vous pouvez lui transmettre cette référence directement : https://wa.me/212650512222')};
    }
    ...
    const constraints = [...safe.history.filter(h => h.role === 'user').map(h => h.text), safe.message].join(' ');
    const schoolBag = /cartable|xkara|chkara|شكارة/i.test(constraints) && /mdrassa|madrassa|scolaire|ecole|école|مدرس/i.test(constraints);
    const adultPuzzle = /puzzle/i.test(constraints) && /adult/i.test(constraints);
    const suitable = p => !(schoolBag && /sacoche|porte.?pc|porte.?ordinateur|briefcase/i.test(p.title)) && !(adultPuzzle && /enfant|educatif|éducatif/i.test(p.title));
    ...
    let fallbackPool = products.filter(suitable);
    const fallback = () => {
      ...
      if (budgetAmount(safe.message) && !originalFamily && !searchQuery(fallbackTerms).family) return contextReply();
    ...
    try {
      ...
      if (plan.action === 'reply') return { mode: 'ai', reply: handoff(plan.reply.trim() || contextualFallback(safe.message, safe.history)), products: [] };
      const collection = collections.find(c => c.handle === plan.collection);
      const pool = (collection ? await catalog.inCollection(collection.handle) : products).filter(suitable);
      ...
      return { mode: 'ai', reply: handoff(answer.reply), products: selectProducts(candidates, answer.productIds) };
    } catch (error) {
