...
customElements.define('alfajr-assistant', class extends HTMLElement {
  ...
  renderReply(node, text) {
    node.textContent = '';
    const allowed = ['https://wa.me/212650512222', 'https://maps.google.com/?q=55+Ave+Mohammed+es+Slaoui,+Fes+30050', 'https://al-fajr.ma/pages/livraison', 'https://al-fajr.ma/pages/politique-retour'];
    for (const part of text.split(/(https:\/\/[^\s]+)/g)) {
      if (allowed.includes(part)) {
        const link = document.createElement('a'); link.href = part;
        link.textContent = part.includes('wa.me') ? 'WhatsApp ↗' : part.includes('maps.google') ? 'Google Maps ↗' : part.includes('livraison') ? 'Livraison ↗' : 'Politique de retour ↗';
        link.target = '_blank'; link.rel = 'noopener noreferrer'; node.append(link);
  ...
  card(product) {
    ...
    this.compareControl?.(info, product);
